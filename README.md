...
- 학생에게 보이는 Student Desktop과 알림 영역 상태, 현재 앱·창 제목·배터리·네트워크 제공
- 학생 선택/전체 대상 메시지, HTTPS URL, 집중 모드, 승인 앱 실행
- 온라인 학생 PC 대상 Windows 잠금(Windows+L), 종료, 재시작 제어와 확인 기록
...
구현하지 않습니다. 학생용 화면에 상태 공유 사실을 표시하고, 교사 명령은 메시지·HTTPS
URL·집중 모드·승인 앱으로 제한합니다. 브라우저 도메인은 관리형 상태 제공기가 연결된
경우에만 표시되며, 현재 창 제목은 학생용 상태 화면과 교사 콘솔에 함께 표시됩니다.

전원·잠금 도구의 종료·재시작·잠금은 온라인 장치에만 실행됩니다. 완전히 꺼진 PC를
켜는 것은 학생 서비스가 명령을 받을 수 없으므로 BIOS/UEFI WOL과 학교 LAN 중계기가
필요하며, 중계기 없이 켜기 동작을 성공한 것처럼 처리하지 않습니다.
