...
자동 시작과 실패 시 재시작으로 등록되고, 서비스가 부팅 때 먼저 실행된 뒤 활성
사용자 세션에서 감시 프로세스를 자동으로 복구합니다. Run 키가 학교 정책으로
차단되거나 학생 화면이 종료되어도 서비스가 최대 약 20초 안에 다시 시작합니다.
전원이 꺼져 있던 PC도 전원을 켜고 Windows에 로그인하면 같은 복구 경로를 탑니다.
등록 정보는
