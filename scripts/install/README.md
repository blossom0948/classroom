...
설치가 끝나면 학생 화면은 작업 표시줄 알림 영역과 시작 메뉴의 `Classroom Student`에
남습니다. 창의 닫기 버튼은 종료가 아니라 상태 창 숨기기로 동작하므로 다시 설치할
필요 없이 시작 메뉴나 알림 영역에서 다시 열 수 있습니다. Windows 서비스는 자동
시작으로 등록되고, 사용자가 로그인하면 감시 프로세스가 자동으로 학생 화면을
실행합니다. 학생 화면 프로세스가 강제로 종료되어도 로그인 세션의 Classroom 감시
프로세스가 최대 약 2초 안에 다시 실행합니다. 같은 학생 코드를 재입력해도 서버는
새 학생 카드를 만들지 않고 기존 학생 장치 기록을 갱신합니다.
