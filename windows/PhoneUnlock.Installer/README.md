...
설치 시 선택한 `Phone Unlock 자동 잠금` 시작 항목은 로그인 후 자동잠금 Agent를 Windows 트레이에 띄웁니다. 트레이 아이콘을 두 번 누르면 설정을 열고, 메뉴에서 PC 잠금을 실행할 수 있습니다. 설정 창을 닫아도 로그인 서비스와 자동잠금 Agent는 계속 실행됩니다.

일반 Windows 설치 마법사가 서비스, 자동 시작, 로컬 네트워크 방화벽 규칙, 시작 메뉴 바로가기를 설치합니다. 실패하면 설치 마법사가 오류 코드를 표시하고 `/LOG=파일경로` 옵션으로 전체 로그를 남길 수 있습니다.

설정 창 자체는 백그라운드 앱이 아니므로 닫아도 된다. 실제 연결은 자동 시작 Windows 서비스가 계속 담당한다. 설정을 다시 열려면 바탕 화면의 **Phone Unlock 설정** 또는 시작 메뉴의 **Phone Unlock > Phone Unlock 설정**을 실행한다.

설정 앱에서 QR 연결 → 현재 계정 암호 확인 → **휴대폰 인증 로그인 켜기**를 진행합니다. 마지막 버튼이 실제 휴대폰 인증 테스트와 로그인 활성화를 함께 처리합니다.
