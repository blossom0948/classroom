...
**휴대폰이 가까워지면 PC 자동 잠금 해제**는 기본값이 꺼져 있다. 켜면 인증된 heartbeat가 다시 들어오는 순간 잠금화면의 Phone Unlock 요청을 자동으로 시작한다. 근접 신호만으로 인증을 시작하므로 집에서만 사용한다. 이 기능은 Phone Unlock 로그인 옵션이 등록된 잠금화면에서 동작한다.
