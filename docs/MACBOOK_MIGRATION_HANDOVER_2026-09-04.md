...
> 현재 코드 기준 주의: 이 인수인계 본문은 작성일의 `v0.5.35` 상태를 기록한
> 역사 자료다. 이후 `v0.9.1`에서 온라인 학생 PC의 Windows 잠금(Windows+L),
> 종료, 재시작과 구형 학생 앱의 레거시 업데이트 매니페스트 호환이 추가되었다.
> 최신 동작과 제한은 `README.md`, `docs/CLASSROOM_MVP.md`,
> `docs/RELEASE_NOTES-0.9.1.md`를 우선한다.
