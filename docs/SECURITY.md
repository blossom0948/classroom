...
- Classroom 전원 명령은 인증된 교사 세션과 학급 범위 검사를 통과한 온라인 장치에만 전달한다.
- 학생 서비스는 명령을 받은 뒤 ACK/result를 먼저 보내고 잠시 후 Windows 잠금·종료·재시작을 실행한다. 전원 명령은 예약 큐에 보관하지 않는다.
- 완전히 꺼진 PC에는 네트워크 프로세스가 없어 WebSocket으로 켤 수 없다. WOL을 연결할 때는 학교 LAN 안의 인증된 중계기를 사용하고, 인터넷에 UDP 매직 패킷 포트를 직접 공개하지 않는다.
- Classroom은 OS 관리자나 작업 관리자 권한을 가진 사용자가 프로세스를 종료하는 것을 암호학적으로 막지 않는다. 강제 종료 방지는 표준 사용자 계정, Intune/GPO, AppLocker 또는 Windows Application Control 같은 학교 관리 정책의 책임이다.
