...
첫 실행 후 Teacher Console의 설정에서 비밀번호를 변경한다. 운영 콘솔은 기존
호환성을 유지하는 bearer 세션을 사용하며 토큰은 계정 로그인에서만 로컬 저장소에
보관한다. 게스트 세션은 세션 저장소에만 보관되어 브라우저를 닫으면 사라진다.
Cloudflare Pages 프록시를 별도로 활성화하는 배포에서는 HttpOnly 세션 쿠키 모드도
지원한다. 개발 기본 계정은 파일럿 전용이며 외부에 노출하지 않는다.
